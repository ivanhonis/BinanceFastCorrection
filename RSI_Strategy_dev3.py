...
class RSIStrategy(bt.Strategy):
    ...
    def next(self):
        ...
        std = np.std(np.array(self.cd.rsi_history)[0:self.std_period])
        # rsi_mean = np.mean(np.array(self.cd.rsi_history)[:self.dev_min])
        # rsi_mean2 = np.std(np.array(self.cd.rsi_history)[:self.dev_max])
        over_sold = 50 - std * 10
        over_bought = 50 + std * 10
        ...
        if not self.live_run or self.is_live_data():
            if self.steps_count >= self.MINIMUM_STEPS:
                ...
                if not self.position:
                    ...
                    if self.cd.rsi_history[1] <= self.cd.rsi_over_sold[0] < self.cd.rsi_history[0]:
                        if self.live_run:
                    ...
                    if self.cd.rsi_history[1] >= self.cd.rsi_over_bought[0] > self.cd.rsi_history[0]:
